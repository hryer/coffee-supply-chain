  ...
  RetailerRole
{
  ...
  struct Item {
    ...
    address itemOwnerID; // Metamask-Ethereum address of the current owner as the product moves through 8 stages
  ...
  modifier onlyContractOwner() {
    require(msg.sender == contractOwner, 'Only owner of the contract');
    _;
  }
  ...
  function kill() public {
    if (msg.sender == contractOwner) {
      address payable payableContractOwner = payable(contractOwner);

      selfdestruct(payableContractOwner);
    }
  }
  ...
  function harvestItem(
    ...
    items[_upc] = Item({
      ...
      itemOwnerID: contractOwner, // Metamask-Ethereum address of the current owner as the product moves through 8 stages
  ...
  // Call modifier to check if upc has passed previous supply chain stage
  // Call modifier to verify caller of this function
  function sellItem(uint256 _upc, uint256 _price) public packed(_upc) onlyFarmer {
    // Update the appropriate fields
  ...
  function buyItem(uint256 _upc) public payable forSale(_upc) paidEnough(items[_upc].productPrice) checkValue(_upc) onlyDistributor {
    ...
    address payable payableOriginFarmerID = payable(items[_upc].originFarmerID);
    payableOriginFarmerID.transfer(items[_upc].productPrice);
    // emit the appropriate event
    emit Sold(_upc);
  }
